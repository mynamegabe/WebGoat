...
public class SimpleMailAssignment implements AssignmentEndpoint {
  ...
  public AttackResult login(
    ...
    // The password is never derived from the user name, it is compared against the randomly
    // generated value which belongs to the account of the currently logged in user.
  ...
  private AttackResult sendEmail(String username, String email, String webGoatUsername) {
    if (username.equals(webGoatUsername)) {
      // A new password is generated with a secure random generator instead of being derived from
      // the user name, and it is never sent over e-mail. E-mail is not a confidential channel, so
      // the message only notifies the user that a reset took place.
      passwords.put(webGoatUsername, createPassword());
